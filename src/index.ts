...
console.error(`🦁 [BRAVE] Process starting - PID: ${process.pid}, Node: ${process.version}, Platform: ${process.platform}`);
console.error(`🦁 [BRAVE] Working directory: ${process.cwd()}`);
console.error(`🦁 [BRAVE] Command args: ${process.argv.join(' ')}`);

// Check for server mode from command line arguments
const args = process.argv.slice(2);

// Support both --mode=http and --mode http formats
let serverMode = 'stdio';
...
for (let i = 0; i < args.length; i++) {
  ...
  if (arg.startsWith('--mode=')) {
    serverMode = arg.split('=')[1];
  } else if (arg === '--mode' && i + 1 < args.length) {
    serverMode = args[i + 1];
  } else if (arg.startsWith('--port=')) {
    serverPort = parseInt(arg.split('=')[1]);
  } else if (arg === '--port' && i + 1 < args.length) {
    serverPort = parseInt(args[i + 1]);
  } else if (arg.startsWith('--host=')) {
    process.env.HTTP_HOST = arg.split('=')[1];
    process.env.WS_HOST = arg.split('=')[1];
  } else if (arg === '--host' && i + 1 < args.length) {
    process.env.HTTP_HOST = args[i + 1];
    process.env.WS_HOST = args[i + 1];
  }
}

console.error(`🦁 [BRAVE] Server mode: ${serverMode}`);
if (serverPort) console.error(`🦁 [BRAVE] Server port: ${serverPort}`);
...
// Import HTTP MCP server
import HttpMcpServer from './http-mcp-server.js';

// Main function to start the server
async function main(): Promise<void> {
  console.error('🦁 [BRAVE] Main function starting...');
  console.error(`🦁 [BRAVE] Selected server mode: ${serverMode}`);
  
  // Handle different server modes
  if (serverMode === 'http' || serverMode === 'websocket') {
    console.error(`🦁 [BRAVE] Starting ${serverMode.toUpperCase()} MCP Server...`);
    
    const httpServer = new HttpMcpServer();
    
    if (serverMode === 'http') {
      await httpServer.start(serverPort || 3000, serverPort ? serverPort + 1 : 3001);
    } else {
      await httpServer.start(serverPort ? serverPort - 1 : 3000, serverPort || 3001);
    }
    
    console.error('🦁 [BRAVE] HTTP/WebSocket MCP Server started successfully!');
    console.error('🦁 [BRAVE] All original Puppeteer features are available via HTTP/WebSocket!');
    return;
  }
  
  // Default STDIO mode
  console.error('🦁 [BRAVE] Starting STDIO MCP Server (default mode)...');
...
console.error('🔍 [DEBUG] Setting up process cleanup handlers...');
  ...
  await withErrorHandling(async () => {
    ...
    console.error('🚀 Brave Puppeteer Real Browser MCP Server started successfully');
    ...
    console.error('🦁 Browser: Using Brave instead of Chrome for better Linux ARM64 support');
