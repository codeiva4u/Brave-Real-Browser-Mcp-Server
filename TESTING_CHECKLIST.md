...
## Purpose
This document provides detailed testing procedures for the brave-real-browser MCP server using Claude Code CLI. Use this checklist for regression testing, pre-publication validation, and ongoing development verification.
...
```
Check if the brave-real-browser MCP server is properly configured and recognized. Use the /mcp command to show server status and list all available tools.
```
...
**Prompt for Claude Code CLI:**
```
List all available tools from the brave-real-browser MCP server and describe what each tool does. Verify that all 11 expected tools are present and have proper descriptions.
```
...
**Prompt for Claude Code CLI:**
```
Use the brave-real-browser MCP server to:
...
```
I need you to perform a comprehensive test of the brave-real-browser MCP server. Please execute the following test sequence and report on each step:

PHASE 1 - Basic Setup:
1. Check MCP server status with /mcp command
2. List all available brave-real-browser tools
...
```
Perform a quick smoke test of the brave-real-browser MCP server:
...
echo "Use brave-real-browser to navigate to example.com and take a screenshot" | claude

# Original error scenario
echo "Use brave-real-browser to navigate to cursor.com/dashboard and screenshot (test retry logic)" | claude

# Error recovery test
echo "Test brave-real-browser error handling by clicking non-existent elements" | claude
```
