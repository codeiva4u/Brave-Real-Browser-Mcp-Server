...
(async () => {
  ...
  if (!packagePath) {
    ...
    // If not found, check parent directories (monorepo support)
    try {
      await access(packagePath, constants.F_OK);
    } catch {
      // Try root node_modules (for monorepo/workspace setups)
      const rootPath = resolve(process.cwd(), '../../node_modules', packageName);
      try {
        await access(rootPath, constants.F_OK);
        packagePath = rootPath;
        log(`📦 Found package in monorepo root: ${rootPath}`);
      } catch {
        // Keep original path, will fail with proper error later
      }
    }
  }
