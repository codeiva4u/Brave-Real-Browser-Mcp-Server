...
- **CDP-first approach** (exclusively using DevTools Protocol to prevent stealth conflicts)
...
**This implementation exclusively uses the CDP (DevTools Protocol) method for all screenshot operations** to prevent conflicts with brave-real-browser's stealth plugins. Standard Puppeteer screenshot methods can interfere with Rebrowser patches, causing detection or failures.
...
1. **CDP-Only Strategy**: Exclusively uses DevTools Protocol (CDP) to prevent stealth conflicts
